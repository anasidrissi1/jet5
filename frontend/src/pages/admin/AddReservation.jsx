...
import { carsService, clientsService, reservationsService } from '../../services/api';
...
const addDaysToIsoDate = (isoDate, daysToAdd) => {
  ...
  if (Number.isNaN(date.getTime())) return '';
  const safeDays = Math.max(0, parseInt(daysToAdd, 10) || 0);
  const next = new Date(date);
  next.setDate(next.getDate() + safeDays);
  return next.toISOString().slice(0, 10);
};

const computeSpecialMonths = (dateDebut, dateFin) => {
  if (!dateDebut || !dateFin) return 1;
  const start = new Date(dateDebut);
  const end = new Date(dateFin);
  if (Number.isNaN(start.getTime()) || Number.isNaN(end.getTime())) return 1;
  const deltaDays = Math.max(0, Math.round((end - start) / (24 * 60 * 60 * 1000)));
  return Math.max(1, Math.ceil(deltaDays / 30));
};
...
const sanitizeExtras = (raw) => {
  ...
  const next = { ...emptyExtras };

  Object.entries(emptyExtras).forEach(([key, defaultValue]) => {
    const value = source[key];
    if (typeof defaultValue === 'boolean') {
      next[key] = Boolean(value);
      return;
    }
    next[key] = value == null ? defaultValue : String(value);
  });
...
function AddReservation() {
      ...
      try {
        ...
        if (isEdit) {
          ...
          setCore((prev) => ({
            ...
            jours_prolongation: r.jours_prolongation != null ? String(r.jours_prolongation) : '0',
          ...
          if (contract) {
            setExtras(sanitizeExtras(contract));
          }
        }
      } catch (error) {
        console.error(error);
  ...
  useEffect(() => {
    ...
    setExtras((prev) => {
      const nextNumeroPermis = selectedClient.permis_numero || '';
      const nextDateDelivrance = toInputDate(selectedClient.permis_date_delivrance);
      const nextDateExpiration =
        toInputDate(selectedClient.permis_date_expiration) ||
        toInputDate(selectedClient.cin_date_expiration) ||
        '';

      return {
        ...prev,
        locataire_cin: selectedClient.cin || selectedClient.cin_numero || '',
        locataire_adresse: selectedClient.adresse || selectedClient.ville || '',
        locataire_numero_permis: nextNumeroPermis,
        locataire_delivre_le: nextDateDelivrance,
        locataire_expiration_permis: nextDateExpiration,
        locataire_passport: selectedClient.passeport_numero || '',
      };
    });
  }, [selectedClient]);
  ...
  useEffect(() => {
    ...
    setExtras((prev) => {
      const nextNumeroPermis = selectedSecondaryClient.permis_numero || '';
      const nextDateDelivrance = toInputDate(selectedSecondaryClient.permis_date_delivrance);
      const nextDateExpiration =
        toInputDate(selectedSecondaryClient.permis_date_expiration) ||
        toInputDate(selectedSecondaryClient.cin_date_expiration) ||
        '';

      return {
        ...prev,
        secondaire_nom: selectedSecondaryClient.nom || '',
        secondaire_prenom: selectedSecondaryClient.prenom || '',
        secondaire_telephone: selectedSecondaryClient.telephone || '',
        secondaire_cin: selectedSecondaryClient.cin || selectedSecondaryClient.cin_numero || '',
        secondaire_adresse: selectedSecondaryClient.adresse || selectedSecondaryClient.ville || '',
        secondaire_numero_permis: nextNumeroPermis,
        secondaire_delivre_le: nextDateDelivrance,
        secondaire_expiration_permis: nextDateExpiration,
        secondaire_passport: selectedSecondaryClient.passeport_numero || '',
      };
    });
  ...
  const totals = useMemo(() => {
    ...
    const extensionDays = Math.max(0, parseInt(core.jours_prolongation, 10) || 0);
    const totalDays = days + extensionDays;
    const price = parseFloat(core.prix_journalier) || 0;
    const specialRate = parseFloat(core.tarif_special) || 0;
    const specialMonths = computeSpecialMonths(core.date_debut, core.date_fin);
    const extra = parseFloat(extras.service_extra) || 0;
    const tva = parseFloat(extras.tva_rate) || 0;
    const base = specialRate > 0 ? specialRate * specialMonths : totalDays * price;
    const ht = base + extra;
    const tvaValue = (ht * tva) / 100;
    const ttc = ht + tvaValue;
    const avance = parseFloat(core.avance) || 0;
    const reste = Math.max(0, ttc - avance);
    return { ht, tvaValue, ttc, reste, specialMonths, usesSpecialRate: specialRate > 0, totalDays };
  }, [core.avance, core.date_debut, core.date_fin, core.jours_prolongation, core.nombre_jours, core.prix_journalier, core.tarif_special, extras.service_extra, extras.tva_rate]);
  ...
  const handleCore = (event) => {
    ...
    setCore((prev) => {
      ...
      } else if (name === 'tarif_special') {
        const parsed = parseFloat(value);
        if (Number.isFinite(parsed) && parsed > 0) {
          next.prix_journalier = '0';
        }
      } else if (name === 'prix_journalier') {
        const parsed = parseFloat(value);
        if (Number.isFinite(parsed) && parsed > 0) {
          next.tarif_special = '';
        }
      ...
      // Synchroniser automatiquement la date de prolongation
      // quand les délais de location changent.
      if (['date_debut', 'date_fin', 'nombre_jours', 'jours_prolongation'].includes(name)) {
        const extensionDays = Math.max(0, parseInt(next.jours_prolongation, 10) || 0);
        const effectiveDateFin = next.date_fin || computeDateFin(next.date_debut, next.nombre_jours);
        const autoProlongationDate = extensionDays > 0 && effectiveDateFin
          ? addDaysToIsoDate(effectiveDateFin, extensionDays)
          : '';

        setExtras((prevExtras) => ({
          ...prevExtras,
          prolongation_date: autoProlongationDate,
        }));
      }
  ...
  const validate = () => {
    ...
    const hasDaily = (parseFloat(core.prix_journalier) || 0) > 0;
    const hasSpecial = (parseFloat(core.tarif_special) || 0) > 0;
    if (!hasDaily && !hasSpecial) {
      addNotification('Prix journalier ou tarif spécial obligatoire.', 'warning');
      return false;
    }
    return true;
  };
  ...
  const handleSave = async (event) => {
    ...
    try {
      const payload = {
        ...
        jours_prolongation: Math.max(0, parseInt(core.jours_prolongation, 10) || 0),
        prix_journalier: (parseFloat(core.tarif_special) || 0) > 0 ? 0 : (parseFloat(core.prix_journalier) || 0),
        tarif_special: (parseFloat(core.tarif_special) || 0) > 0 ? parseFloat(core.tarif_special) : null,
        tarif_special_unite: 'PAR_MOIS',
        avance: parseFloat(core.avance) || 0,
        franchise: parseFloat(core.franchise) || 0,
        long_duration: false,
        billing_mode: (parseFloat(core.tarif_special) || 0) > 0 ? 'FORFAIT' : 'JOURNALIER',
...
<div class="head">
  <div>
    <div class="bold" style="font-size:20px;">JET5</div>
...
<div class="section">
  ...
  <div class="grid">
    ...
    <div class="row"><div>Téléphone</div><div>${selectedClient?.telephone || '—'}</div></div>
...
<div class="section">
  ...
  <div class="fact-grid">
    ...
    <div><span class="bold">Tarif spécial (par mois)</span><br/>${displayMoney(core.tarif_special)}</div>
    <div><span class="bold">Mode facturation</span><br/>${totals.usesSpecialRate ? 'Forfait mensuel' : 'Journalier'}</div>
    <div><span class="bold">Durée</span><br/>${core.nombre_jours} jour(s)</div>
    <div><span class="bold">Périodes</span><br/>${totals.specialMonths} mois</div>
...
<script>window.onload = () => window.print();</script>
  ...
  const handlePdf = async () => {
    ...
    try {
      ...
      y += lh;
      line('Nom/Prenom', `${selectedClient?.nom || ''} ${selectedClient?.prenom || ''}`.trim());
      line('Telephone', selectedClient?.telephone || '');
      ...
      line('Tarif special (par mois)', displayMoney(core.tarif_special));
      line('Mode facturation', totals.usesSpecialRate ? 'Forfait mensuel' : 'Journalier');
      line('Duree location', `${core.nombre_jours} jour(s)`);
      line('Periodes', `${totals.specialMonths} mois`);
      ...
      <form className="contract-form" onSubmit={handleSave}>
        ...
        <section className="contract-section">
          ...
          <div className="grid-4">
            ...
            <label>Jours prolongation<input type="number" min="0" name="jours_prolongation" value={core.jours_prolongation} onChange={handleCore} /></label>
            <label>Date prolongation<input type="date" name="prolongation_date" value={extras.prolongation_date} onChange={handleExtras} /></label>
            <label>Heure prolongation<input type="time" name="prolongation_heure" value={extras.prolongation_heure} onChange={handleExtras} /></label>
            <label>KM prolongation<input name="prolongation_km" value={extras.prolongation_km} onChange={handleExtras} placeholder="Ex: 58210" /></label>
            <label>Lieu prolongation<input name="prolongation_lieu" value={extras.prolongation_lieu} onChange={handleExtras} placeholder="Ex: Agence Casa" /></label>
          </div>
        </section>
        ...
        <section className="contract-section">
          ...
          <div className="grid-4">
            <label>
              Tarif jour TTC
              <input name="prix_journalier" value={core.prix_journalier} onChange={handleCore} disabled={(parseFloat(core.tarif_special) || 0) > 0} />
            </label>
            <label>
              Tarif spécial (par mois)
              <input name="tarif_special" value={core.tarif_special} onChange={handleCore} placeholder="Ex: 9000" />
            </label>
            ...
            <label className="readonly">Durée totale<input value={`${totals.totalDays} jour(s)`} readOnly /></label>
