...
function Payments() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await paymentsService.financialSummary();
      const data = res.data || {};
  ...
  const styles = {
    ...
    actionBtn: {
      ...
      background: 'linear-gradient(135deg, rgba(239,68,68,0.12), rgba(248,113,113,0.16))',
      color: 'var(--text-primary)',
      fontWeight: 700,
      fontSize: 13,
      cursor: 'pointer',
    ...
    inlineMsg: (tone) => ({
      padding: '10px 12px',
      borderRadius: 10,
      border: `1px solid ${tone === 'error' ? 'rgba(248,113,113,0.4)' : 'rgba(34,197,94,0.35)'}`,
      background: tone === 'error' ? 'rgba(248,113,113,0.08)' : 'rgba(34,197,94,0.08)',
  ...
  const handleSubmitPayment = async () => {
    ...
    try {
      await paymentsService.addPayment(paymentId, payload);
      await fetchData();
  ...
  useEffect(() => {
    const loadHistory = async () => {
      ...
      setHistoryError(null);
      try {
        const res = await paymentsService.history(paymentId);
        setHistory(res.data || []);
      } catch (err) {
    ...
    <div className="payments-page" style={styles.page}>
      <div style={styles.shell}>
        ...
        <div style={styles.tableWrap}>
          ...
          {!loading && !error && (
            <table className="payments-table" style={styles.table}>
              <thead>
                <tr>
