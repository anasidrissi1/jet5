...
function VisitesTechniques() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const visitesRes = await apiClient.get('/cars/visites/');
  ...
  const confirmDelete = async () => {
    ...
    try {
      await apiClient.delete(`/cars/visites/${pendingDelete.id}/`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await apiClient.post('/cars/visites/', dataToSend);

      // Refresh data
      const res = await apiClient.get('/cars/visites/');
                ...
                <tbody style={{ background: 'var(--bg-card)' }}>
                  {filtered.map(visite => {
                    ...
                    return (
                      ...
                      >
                        <td style={{ padding: '16px', fontSize: '14px', fontWeight: '500' }}><span style={{ fontSize: '13px', color: '#f5c400', background: 'rgba(245, 196, 0, 0.14)', borderRadius: '6px', padding: '2px 8px', display: 'inline-block' }}>{car?.immatriculation || '-'}</span></td>
                        ...
                        <td style={{ padding: '16px', textAlign: 'center' }}><span style={{ background: 'rgba(245, 196, 0, 0.14)', color: '#8a6d00', padding: '6px 18px', borderRadius: '20px', fontWeight: '700', fontSize: '14px', display: 'inline-block', minWidth: '90px', textAlign: 'center' }}>{stat.emoji} {stat.status}</span></td>
            ...
            >
              <div
                style={{
                  ...
                  height: '64px',
                  borderRadius: '18px',
                  background: 'linear-gradient(135deg, rgba(245, 196, 0, 0.18) 0%, rgba(212, 169, 0, 0.24) 100%)',
                  border: '1px solid rgba(212, 169, 0, 0.4)',
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'center',
                  fontSize: '28px',
              ...
              <p style={{ margin: '0 0 20px', lineHeight: 1.6, color: '#FDE68A' }}>
                La visite associée au véhicule{' '}
                <strong style={{ color: '#FCD34D' }}>{pendingDelete.car?.immatriculation || 'non renseigné'}</strong>{' '}
