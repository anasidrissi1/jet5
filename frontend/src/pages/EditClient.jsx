...
function EditClient() {
  ...
  const sectionStyle = {
    marginBottom: '2rem',
    padding: '1.5rem',
    background: isDarkMode ? 'rgba(30, 41, 59, 0.8)' : 'var(--gray-50)',
    borderRadius: 'var(--radius-lg)',
    border: isDarkMode ? '1px solid rgba(148, 163, 184, 0.2)' : '1px solid var(--border-color)'
  };
  ...
  const sectionTitleStyle = {
    marginBottom: '1rem',
    color: isDarkMode ? '#f1f5f9' : 'var(--text-primary)',
  ...
  const inputStyle = {
    width: '100%',
    padding: '12px 16px',
    border: isDarkMode ? '2px solid rgba(148, 163, 184, 0.3)' : '2px solid var(--border-color)',
    borderRadius: '10px',
    fontSize: '15px',
    outline: 'none',
  ...
  const hintStyle = {
    display: 'block',
    marginTop: '0.5rem',
    color: isDarkMode ? '#94a3b8' : 'var(--text-tertiary)',
    fontSize: '12px'
  };
      ...
      <div className="cars-body">
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom: '2rem' }}>
            <h3 style={{ marginBottom: '1.5rem', color: isDarkMode ? '#f1f5f9' : 'var(--text-primary)', fontSize: 'var(--font-size-lg)' }}>
              👤 Informations Personnelles
            </h3>
          ...
          <div style={{ marginTop: '2rem', paddingTop: '2rem', borderTop: isDarkMode ? '2px solid rgba(148, 163, 184, 0.2)' : '2px solid var(--border-color)' }}>
            <h3 style={{ marginBottom: '1.5rem', color: isDarkMode ? '#f1f5f9' : 'var(--text-primary)', fontSize: 'var(--font-size-lg)' }}>
              📄 Documents d'Identité
            </h3>
              ...
              <div className="responsive-grid-240" style={{ gap: '1.5rem' }}>
                ...
                <div>
                  ...
                  {existingFiles.cin_documents.length > 0 && (
                    <div style={{ marginBottom: '0.5rem', padding: '0.5rem', background: isDarkMode ? 'rgba(59, 130, 246, 0.15)' : 'var(--info-light)', borderRadius: 'var(--radius-sm)', fontSize: 'var(--font-size-sm)', display: 'grid', gap: '0.45rem' }}>
                      {existingFiles.cin_documents.map((file) => (
                        <a
                          key={file.id}
                          href={file.url}
                          onClick={(event) => { event.preventDefault(); window.open(file.url, '_blank', 'noopener,noreferrer'); }}
                          style={{ color: isDarkMode ? '#FDE68A' : 'var(--info-color)', textDecoration: 'none', cursor: 'pointer' }}
                        >
                          📄 {file.name}
                        </a>
                      ))}
                  ...
                  <DocumentUploadField
                    ...
                    removeButtonStyle={{ background: 'none', border: 'none', color: '#FCD34D', cursor: 'pointer', padding: '0.25rem' }}
                    hintStyle={hintStyle}
