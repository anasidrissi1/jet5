...
function Assurances() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const assurancesRes = await apiClient.get("/cars/assurances/");
  ...
  const getStatus = (dateExpiration) => {
    ...
    if (diffDays <= 15) {
      return { status: "en attente", color: "warning", emoji: "⚠️", days: diffDays };
    }
  ...
  const filtered = useMemo(() => {
    ...
    return normalizedAssurances.filter((assurance) => {
      ...
      const matchesStatus =
        ...
        (filter === "expiring" && statusInfo.status === "en attente") ||
  ...
  const confirmDelete = async () => {
    ...
    try {
      await apiClient.delete(`/cars/assurances/${pendingDelete.id}/`);
                      ...
                      <tr key={assurance.id}>
                        ...
                        <td>
                          {statusInfo.status === "expiré" ? (
                            <span className="text-danger">Expirée depuis {statusInfo.days} j</span>
                          ) : (
                            <span>{statusInfo.days} jour(s)</span>
                          )}
                        </td>
                        <td>
