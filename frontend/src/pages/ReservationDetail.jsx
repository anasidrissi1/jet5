...
const extractNotes = (commentaire) => {
  ...
  return String(commentaire)
    .split('\n')
    .map((line) => line.trim())
    .filter((line) => line && !line.startsWith('[PA_INIT]') && !line.startsWith('[PA_AUDIT]'))
    .join('\n');
};
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const resReservation = await apiClient.get(`/reservations/${id}/`);
        setReservation(resReservation.data);

        // Charger les paiements associés
        try {
          const resPayments = await apiClient.get(`/payments/`, { params: { reservation: id } });
          ...
          if (payments.length > 0) {
            ...
            try {
              const historyData = await paymentService.getPaymentHistory(payments[0].id);
